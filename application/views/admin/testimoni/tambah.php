...
<?php
// application/views/admin/testimoni/tambah.php
...
<div class="bg-white rounded-lg shadow-md">
    ...
    <div class="p-6 border-b border-gray-200">
        <div class="flex justify-between items-center">
            <h3 class="text-xl font-semibold text-gray-800">Tambah Testimoni Baru</h3>
    ...
    <!-- Form -->
    <div class="p-6">
        <?= form_open_multipart('testimoni/tambah', ['class' => 'space-y-6']) ?>
                    ...
                    <div class="flex items-center space-x-2">
                        <?php for($i = 1; $i <= 5; $i++): ?>
                            <label class="cursor-pointer">
                                <input type="radio" name="rating" value="<?= $i ?>" class="sr-only rating-input" <?= set_radio('rating', $i) ?>>
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="5"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-secom-blue-light focus:border-transparent outline-none"
                          placeholder="Masukkan testimoni atau review" required><?= set_value('deskripsi') ?></textarea>
...
<?php $this->load->view('templates/admin_footer'); ?>600 transition-colors duration-200">
                    Batal
                </a>
                <button type="submit" 
                        class="bg-secom-blue-dark text-white px-6 py-3 rounded-lg hover:bg-secom-blue-light transition-colors duration-200">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set initial state if there's a selected rating
    const checkedInput = document.querySelector('.rating-input:checked');
    if (checkedInput) {
        updateStars(parseInt(checkedInput.value));
    }
